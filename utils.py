...
# I (Isaiah) changed the above two functions to add MPS (Apple silicon)
# support, just in case my changes broke things, here're the old
# function. Right now these're unused, obviously.
def set_seed_OLD(args):
    random.seed(args.seed)
    np.random.seed(args.seed)
    torch.manual_seed(args.seed)
    if args.n_gpu > 0:
